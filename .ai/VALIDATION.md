...
**Criterio de sucesso**:
- Resultado: `OK`.
- Quantidade de testes esperada (baseline 2026-07-30): **12 testes**.
  - Etapa 9 (reorganizacao dos testes) reorganiza os mesmos 12 testes em
    arquivos menores; o numero total permanece 12.

### 3. Checagem de imports quebrados

Apos cada etapa (principalmente a partir da Etapa 8 — limpeza de shims),
executar verificacao por `grep` para confirmar que nenhum import legado
permanece:

```powershell
# Deve retornar VAZIO apos Etapa 8:
rg "from app\.settings import|from app\.csv_utils import|from app\.business_days import|from app\.email_queue import|from app\.cleanup import|from app\.auth import|from app\.downloader import|from app\.mailer import" app tests tools
```
...
Apos a quebra do `mailer.py` (Etapa 5),.e necessario validar que os 4 HTMLs
...
- Para comparacao completa, antes da Etapa 5 o operador deve salvar (em
  ...
  `.gitignore`) os 4 HTMLs via um script auxiliar de mock de `_send_message`
